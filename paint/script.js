...
const drawRect = (e) => {
    if (colorCheck.checked) {
        return ctx.fillRect(e.offsetX, e.offsetY, pageX - e.offsetX, pageY - e.offsetY);
    }
...
const drawCircle = (e) => {
    ctx.beginPath();
    let radius = Math.sqrt(Math.pow((pageX - e.offsetX), 2) + Math.pow((pageY - e.offsetY), 2));
...
tool.forEach(element => {
    element.addEventListener('click', () => {
        document.querySelector('.top_tools .active_tool').classList.remove('active_tool');
...
color_option.forEach(element => {
    element.addEventListener('click', () => {
        document.querySelector('.ul_color_list .selected_color').classList.remove('selected_color');
        element.classList.add('selected_color');
        selected_Color = window.getComputedStyle(element).getPropertyValue('background-color');
    })
});
